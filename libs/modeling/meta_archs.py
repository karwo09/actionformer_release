...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.fpn_strides = [scale_factor**i for i in range(
            fpn_start_level, backbone_arch[-3]+1
        )]
        ...
        if isinstance(n_mha_win_size, int):
            self.mha_win_size = [n_mha_win_size]*(1 + backbone_arch[-3])
        else:
            assert len(n_mha_win_size) == (1 + backbone_arch[-3])
        ...
        if backbone_type == 'convTransformer':
            ...
            self.backbone = make_backbone(
                ...
                **{
                    ...
                    'audio_pretrained_path' : train_cfg.get('audio_pretrained_path'),
                    'freeze_aformer' : train_cfg['freeze_aformer'],
                    'vconfig_path' : train_cfg['vconfig_path'],
                    'aconfig_path' : train_cfg['aconfig_path'],
                }
            )
        else:
            self.backbone = make_backbone(
        ...
        self.neck = make_neck(
            fpn_type,
            **{
                'in_channels' : [embd_dim] * (backbone_arch[-3] + 1),
                'out_channel' : fpn_dim,
                'scale_factor' : scale_factor,
    ...
    def find_unflatten_H_W(self, tensor):
        assert tensor.dim() == 1
        W, H = 1, tensor.shape[0]
        
        upper_bound = int(math.sqrt(H))
        for w in range(upper_bound, 0, -1):
            if H % w == 0:
                W = H // w
                H = w
                break
        ...
        if self.training:
            ...
            if self.use_audio:
                batched_audio_inputs = F.pad(
                    audio_feats[0], padding_size_a, value=padding_val).unsqueeze(0)
                batched_audio_inputs = batched_audio_inputs
